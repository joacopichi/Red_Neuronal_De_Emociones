...
class CorrectionRepository:
    ...
    def get_correction_by_id(self, correction_id: int) -> Optional[Correction]:
        try:
            return self.db.query(Correction).filter(Correction.id == correction_id).first()
        except Exception as e:
    ...
    def get_feedback_statistics(self) -> Dict:
        try:
            ...
            emotions_distribution = {
                emotion: count for emotion, count in emotion_stats
            }
            ...
            recent_activity = [
                {
                    "id": corr.id,
                    "text": corr.text[:50] + "..." if len(corr.text) > 50 else corr.text,
