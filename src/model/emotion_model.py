...
class EmotionModel:
    def __init__(self, vocab_size: int, num_classes: int, max_len: int,
        ...
        if not reentrenar_desde_cero and os.path.exists(self.model_path):
            ...
            print("⚠️ No se encontró modelo o se solicitó reentrenar desde cero. Construyendo nuevo modelo...")
            self.model = self._build_model()
            self.save()  # guardamos el modelo recién creado
    ...
    def load(path="saved/emotion_model.keras"):
        ...
        if os.path.exists(path):
            ...
            wrapper = EmotionModel(vocab_size=1, num_classes=1, max_len=1, model_path=path)
            wrapper.model = model
            return wrapper
